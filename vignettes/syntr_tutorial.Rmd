...
A simple method of finding the optimal combination of these two parameters is to fit a range of values for both and choose values that maximize coverage (% of the map assigned to synteny blocks) of the two maps. 
...
We employ a simple "elbow" heuristic is to choose the *first* composite coverage "peak" in the parameter matrix. We've also found that `max_nn_dist` has an overall weak effect if your maps are fairly clean, and can result in odd behavior if increased beyond reasonable values. We reccomend focusing on `max_cluster_range`, and choosing the first peak that also minimized max_nn_dist. 

So, focusing on the first row (`max_nn_dist` = 10) below and moving from left to right, we select a `max_cluster_range` of 2 and a `max_nn_dist` of 10. 
